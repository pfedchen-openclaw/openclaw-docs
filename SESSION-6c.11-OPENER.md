...
> **⚠️ STATE NOTE (added 2026-06-27, [P6-77]) — heartbeats are PAUSED; this overrides the Stage-0 "expected state" below.** To stop a ~$20/day GPT-5-fallback bleed while the pair is degraded (Anthropic June cap + expired Google OAuth [F44]), Marie/Charlotte heartbeats were disabled at runtime (`openclaw system heartbeat disable`) and a sentinel `logs/heartbeat.paused` makes `heartbeat-watchdog.sh` **enforce** the disable (re-asserts every 15m, survives a restart) and **suppress** its stall-restart + fallback-alert. **So at open you will correctly see no recent beats and no watchdog Telegrams — that is the pause, not a fault** (the line below's "tail the heartbeat logs; both LaunchAgents loaded" liveness expectation is superseded). **RESUME as part of WS-0, only after the cap is lifted + OAuth re-authed:** `rm logs/heartbeat.paused` **then** `openclaw system heartbeat enable` (that order — else the watchdog re-disables within 15m); then verify a real `[heartbeat]` fires in gateway.log, and fold the temporary watchdog guard into the WS-8 alerting redesign rather than leaving two mechanisms. Anthropic subscription was upgraded 27 Jun → beats return to the Claude plan on resume. Procedure also in RUNBOOK §2.14; full detail in [P6-77].
...
Capabilities (Drive/Sheets write, agent email, browser robustness) → **6c.12**; payments + accounts + credential handling → **6c.13**; family channels (Olga/parents, WS-11) → with the messenger build; **acceptance gate** → **6c.14**; **Travel agent** → **6c.15** (before Andreas); the displaced 6c.10 **capstone — ARCHITECTURE v7 + canonical reconciliation** — re-instated as **6c.16**, immediately before 6d Andreas, authored on the proven roster ([P6-76]). v7 is NOT authored before the pair passes the acceptance gate. Standing: [F59]/[P6-66] rotations (deferred), S6 reconcile, SR-1/D23, F64/F68/F70 launchd, 2026.6.6 (pinned); **model+framework currency** ([P6-74]) — Opus-4.8 / newer-framework upgrade owned by Andreas, pin-bump surfaced to Peter.
