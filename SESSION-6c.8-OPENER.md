...
- **DECIDED (Peter, 6c.7) — ring-fence into two Anthropic workspaces:** **(A) Marie+Charlotte production ≈ $400/mo (protected)**; **(B) other agents + all dev/build/testing ≈ $500/mo**; total ≤ $1000. **Peter's prerequisite action:** create Workspace B + key in the Console, set both caps, store the key in **1Password** (never pasted in chat). **Build:** wire production agents → A, dev/testing → B via SecretRef (mechanism = per-agent key vs dev profile for test turns; F17/F21). The $300 cap was already raised → $500; the ring-fence supersedes a single cap. **Confirm access** with ONE cheap Sonnet turn that the primary model answers (not the fallback) — no Opus, no loops ([P6-51]).
