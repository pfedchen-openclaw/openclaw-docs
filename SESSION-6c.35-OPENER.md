...
2. **Cost-model reconciliation** — when Peter's **Admin API key** lands (0600 `agent-creds/anthropic-admin.key`), run `cost-breakdown.py --agent all --tz utc` → confirm it auto-fetches Console + passes the **±15% gate** (else recalibrate `rates.json`). This closes the flagship acceptance gate ([P6-148]/[P6-85]). If the key is unavailable on the account ([P6-109]), reconcile once via a manual `--console-total` from Peter's Console read and record the delta.
